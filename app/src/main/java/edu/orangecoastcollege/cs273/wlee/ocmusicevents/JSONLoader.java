    ...
    public static ArrayList<MusicEvent> loadJSONFromAsset(Context context) throws IOException {
        ...
        String json = null;
            ...
            json = new String(buffer, "UTF-8");
                // 1. extracting whole thing as a string


        try {
            JSONObject jsonRootObject = new JSONObject(json);
            JSONArray allMusicEventsJSON = jsonRootObject.getJSONArray("MusicEvents"); // extracting root element, go through each object in the array
            int numberOfEvents = allMusicEventsJSON.length();

            for (int i = 0; i < numberOfEvents; i++) {
                JSONObject musicEventJSON = allMusicEventsJSON.getJSONObject(i);

                MusicEvent event = new MusicEvent(); // event, one object?
                event.setTitle(musicEventJSON.getString("Title")); // two versions
                //event.setTitle(musicEventJSON.optString("Title")); optional, returns "" when not found
                event.setDate(musicEventJSON.getString("Date")); // look for key, extract value
